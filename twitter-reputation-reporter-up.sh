...
docker build -t date-agreggator -f src/date_agregator/Dockerfile .

docker-compose up -d reporter-init reporter-filter-parser reporter-analyzer reporter-user-reducer reporter-date-reducer reporter-date-agreggator
